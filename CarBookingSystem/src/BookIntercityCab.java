...
public class BookIntercityCab extends JFrame{
    ...
    public BookIntercityCab(String username){
        ...
        try{
            ...
            ResultSet rs = c.statement.executeQuery("select DISTINCT source, destination from intercity");
            while(rs.next()){
                c1.add(rs.getString("source"));
                c2.add(rs.getString("destination"));
            }

            rs = c.statement.executeQuery("select * from account where username = '"+username+"'");
            while(rs.next()){
        ...
        ImageIcon i1 = new ImageIcon("icons/intracab.png");
        ...
        show.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ae){
                try{
                    ...
                    ResultSet rs = c.statement.executeQuery(str2);
                    if(rs.next()){
                        l6.setText(rs.getString("driver"));
                        l8.setText("Taka " + rs.getString("price"));
                        l4.setText(rs.getString("car"));
                    }

                    rs = c.statement.executeQuery(str);
                    if(rs.next()){
                        l10.setText(rs.getString("name"));
                    }
                    l12.setText(username);
        ...
        b1.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ae){
                try{
                    ...
                    c.statement.executeUpdate("delete from interCab");
