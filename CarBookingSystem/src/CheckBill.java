...
public class CheckBill extends JFrame{
    ...
    public CheckBill(String username) {
        ...
        try{
            ...
            ResultSet rs = c.statement.executeQuery("select * from intercab where username = '"+username+"'");
            while(rs.next()){
                price1 = rs.getString("price");
            }
            rs = c.statement.executeQuery("select * from intracab where username = '"+username+"'");
            while(rs.next()){
                price2 = rs.getString("price");
            }
            String name = "";
            rs = c.statement.executeQuery("select * from customer where username = '"+username+"'");
            while(rs.next()){
                name = rs.getString("name");
            }
            rs = c.statement.executeQuery("select * from intransport where name = '"+username+"'");
            while(rs.next()){
    ...
    public JPanel createPackage(String[] pack){
        ...
        ImageIcon i1  = new ImageIcon("/icons/"+pack[0]);
